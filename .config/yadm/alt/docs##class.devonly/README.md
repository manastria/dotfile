...
| [vault-open.md](vault-open.md) | [`vault-open.sh`](../.local/bin/vault-open.sh), [`vault-close.sh`](../.local/bin/vault-close.sh), [`vault-status.sh`](../.local/bin/vault-status.sh) | Ouverture, fermeture et état quotidiens du coffre LUKS créé par `vault-create.sh` |
