...
curl -s http://repo.code-industry.net/deb/pubmpekey.asc | gpg --dearmor | tee /usr/share/keyrings/pubmpekey.gpg > /dev/null
...
Suites: stable
Components: main
Signed-By: /usr/share/keyrings/pubmpekey.gpg" | tee /etc/apt/sources.list.d/master-pdf-editor.sources > /dev/null
